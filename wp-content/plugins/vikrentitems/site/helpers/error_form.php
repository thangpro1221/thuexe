...
if (VikRentItems::allowRent()) {
	 ...
	 */
	$selform = "<div class=\"vridivsearch vri-main-search-form\"><form action=\"".JRoute::_('index.php?option=com_vikrentitems'.(!empty($pitemid) ? '&Itemid='.$pitemid : ''))."\" method=\"post\" onsubmit=\"return vriValidateSearch();\"><div class=\"vricalform\">\n";
...
jQuery(function(){
	...
	jQuery('.vri-cal-img, .vri-caltrigger').click(function() {
		...
		if (!is_array($vrisessioncart) || !count($vrisessioncart)) {
			...
			if (is_array($timeslots) && count($timeslots) > 0) {
				$selform .= "<div class=\"vrisfentrytimeslot\"><div class=\"vri-sf-timeslot-inner\"><label for=\"vri-timeslot\">".JText::_('VRIFOR') . "</label>";
	...
	$selform .= "<div class=\"vrisfentrysubmit\"><input type=\"submit\" name=\"search\" value=\"" . JText::_('VRISEARCHBUTTON') . "\" class=\"btn vri-search-btn\"/></div>\n";
	$selform .= "</div>\n";
	//locations on google map
	if (count($coordsplaces) > 0) {
		$selform .= '<div class="vrilocationsbox"><div class="vrilocationsmapdiv"><a href="'.JRoute::_('index.php?option=com_vikrentitems&view=locationsmap&tmpl=component').'" class="vrimodal" target="_blank"><i class="' . VikRentItemsIcons::i('map-marked') . '"></i><span>'.JText::_('VRILOCATIONSMAP').'</span></a></div></div>';
		?>
