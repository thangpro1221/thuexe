...
class VikRentItems
{
	...
	public static function getSubmitName($skipsession = false)
	{
		 ...
		 * @deprecated 	from 1.7
		 */
		return JText::_('VRISEARCHBUTTON');
	}
	...
	public static function parseEmailTemplate($tmpl, $bid, $vricart, $total = 0, $link = null)
	{
		...
		if ($order_info['status'] == 'cancelled') {
			...
			$status_str = JText::_('VRIOMPLETED');
		}
		...
		if (!empty($order_info['idplace']) && !empty($order_info['idreturnplace'])) {
			...
			if ($locfee) {
				...
				if (!empty($locfeewith) && $locfeewith > 0) {
					$orderdetails .= '<div class="hireordata"><span class="Stile9">'.JText::_('VRLOCFEETOPAY').'</span><div align="right"><span class="Stile9">'.$currencyname.' '.self::numberFormat($locfeewith).'</span></div></div>';
				}
			}
		}
		...
		if (!empty($order_info['deliverycost']) && $order_info['deliverycost'] > 0) {
			$orderdetails .= '<br/><div class="hireordata"><span class="Stile9">'.JText::_('VRIMAILTOTDELIVERY').'</span><div align="right"><span class="Stile9">'.$currencyname.' '.self::numberFormat($order_info['deliverycost']).'</span></div></div>';
		}
	...
	public static function parsePdfTemplate($tmpl, $bid, $vricart, $total = 0, $link = null)
	{
		...
		if (!empty($order_info['idplace']) && !empty($order_info['idreturnplace'])) {
			...
			if ($locfee) {
				...
				$locfeewith = self::sayLocFeePlusIva($locfeecost, $locfee['idiva'], $order_info);
				if (!empty($locfeewith) && $locfeewith > 0) {
					$orderdetails .= '<tr><td align="left" style="border: 1px solid #DDDDDD;">'.JText::_('VRLOCFEETOPAY').'</td><td align="center" style="border: 1px solid #DDDDDD;">'.$vricart[key($vricart)][0]['infopdf']['days'].'</td><td align="left" style="border: 1px solid #DDDDDD;">'.$currencyname.' '.self::numberFormat($vricart[key($vricart)][0]['infopdf']['locfeenet']).'</td><td align="left" style="border: 1px solid #DDDDDD;">'.$currencyname.' '.self::numberFormat(($locfeewith - $vricart[key($vricart)][0]['infopdf']['locfeenet'])).'</td><td align="left" style="border: 1px solid #DDDDDD;">'.$currencyname.' '.self::numberFormat($locfeewith).'</td></tr>';
	...
	public static function sendCustMail($to, $subject, $ftitle, $ts, $custdata, $vricart, $first, $second, $tot, $link, $status, $place = "", $returnplace = "", $maillocfee = "", $orderid = "", $strcouponeff = "", $totdelivery = 0)
	{
		...
		$attachment = null;
		if ($status == JText::_('VRIOMPLETED') && self::sendPDF() && file_exists(VRI_SITE_PATH . DS . "helpers" . DS . "tcpdf" . DS . 'tcpdf.php')) {
	...
	public static function sendCustMailFromBack($to, $subject, $ftitle, $ts, $custdata, $vricart, $first, $second, $tot, $link, $status, $place = "", $returnplace = "", $maillocfee = "", $orderid = "", $strcouponeff = "", $sendpdf = true, $totdelivery = 0)
	{
		...
		//VikRentItems 1.1 PDF
		$attachment = null;
		if ($status == JText::_('VRIOMPLETED') && $sendpdf && self::sendPDF() && file_exists(VRI_SITE_PATH . DS . "helpers" . DS . "tcpdf" . DS . 'tcpdf.php')) {
			list($pdfcont, $pdfparams) = self::loadPdfTemplate($orderid);
			$pdfhtml = self::parsePdfTemplate($pdfcont, $orderid, $vricart, $tot, $link);
	...
	public static function sendOrderEmail($bid, $for = array(), $send = true, $withpdf = true) {
		...
		foreach ($orderitems as $koi => $oi) {
			...
			$pricestr = ($is_cust_cost ? JText::_('VRIRENTCUSTRATEPLAN').": ".$costplusiva : self::getPriceName($tar[0]['idprice'], $vri_tn).": ".$costplusiva.(!empty($tar[0]['attrdata']) ? "\n".self::getPriceAttr($tar[0]['idprice'], $vri_tn).": ".$tar[0]['attrdata'] : ""));
		...
		// order status
		$saystatus = $booking['status'] == 'confirmed' ? JText::_('VRIOMPLETED') : ($booking['status'] == 'standby' ? JText::_('VRSTANDBY') : JText::_('VRCANCELLED'));
