...
class VikRentItemsController extends JControllerVikRentItems {
		...
		if (!VikRentItems::userIsLogged()) {
			if (!empty($pname) && !empty($plname) && !empty($pusername) && VikRentItems::validEmail($pemail) && $ppassword == $pconfpassword) {
				...
				if ($newuserid!=false && strlen($newuserid)) {
					 ...
					 */
					$mainframe->login($credentials, array('redirect' => JRoute::_('index.php?option=com_vikrentitems&task=oconfirm&'.$qstring, false)));
										...
										if (VikRentItems::areTherePayments()) {
											...
											if (is_array($payment)) {
												...
												if (intval($payment['setconfirmed']) == 1) {
													...
													$mainframe = JFactory::getApplication();
													$mainframe->redirect(JRoute::_("index.php?option=com_vikrentitems&view=order&sid=" . $sid . "&ts=" . $nowts . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
												} else {
												...
												$mainframe = JFactory::getApplication();
												$mainframe->redirect(JRoute::_("index.php?option=com_vikrentitems&place=" . $pplace . "&returnplace=" . $preturnplace . "&days=" . $pdays . "&pickup=" . $ppickup . "&release=" . $prelease . "&task=oconfirm" . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
											}
										} else {
											$realback = VikRentItems::getHoursItemAvail() * 3600;
											$realback += $prelease;
	...
	function cancelrequest() {
		...
		if (!empty($pemail) && !empty($preason)) {
			...
			$mainframe->redirect(JRoute::_("index.php?option=com_vikrentitems&view=order&sid=".$order[0]['sid']."&ts=".$order[0]['ts']."&Itemid=".$pitemid, false));
		} else {
			$mainframe->redirect(JRoute::_("index.php?option=com_vikrentitems&view=order&sid=".$order[0]['sid']."&ts=".$order[0]['ts']."&Itemid=".$pitemid, false));
		}
	}
					...
					if ($array_result['verified'] == 1) {
						...
						if (isset($array_result['tot_paid'])) {
							...
							if ($shouldpay != $totreceived) {
								...
								$vri_app = VikRentItems::getVriApplication();
								$vri_app->sendMail($admail, $admail, $admail, $admail, JText::_('VRITOTPAYMENTINVALID'), JText::sprintf('VRITOTPAYMENTINVALIDTXT', $rows[0]['id'], $totreceived." (".$array_result['tot_paid'].")", $shouldpay), false);
							}
						}
						//
						$extra_emails = array();
						...
						if (!array_key_exists('skip_email', $array_result) || $array_result['skip_email'] != 1) {
							$vri_app = VikRentItems::getVriApplication();
							$vri_app->sendMail($admail, $admail, $admail, $admail, JText::_('VRPAYMENTNOTVER'), JText::_('VRSERVRESP') . ":\n\n" . $array_result['log'], false);
						}
						if (method_exists($obj, 'afterValidation')) {
