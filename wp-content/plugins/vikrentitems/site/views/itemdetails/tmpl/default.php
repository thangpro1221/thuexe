...
if (strlen($item['moreimgs']) > 0) {
	...
	foreach ($moreimages as $mimg) {
		if (!empty($mimg)) {
			?>
					<a href="<?php echo VRI_ADMIN_URI; ?>resources/big_<?php echo $mimg; ?>" rel="vrigroup<?php echo $item['id']; ?>" target="_blank" class="vrimodal" data-fancybox="gallery"><img src="<?php echo VRI_ADMIN_URI; ?>resources/thumb_<?php echo $mimg; ?>" alt="<?php echo substr($mimg, 0, ((int)strpos($mimg, '.') + 1)); ?>"/></a>
			<?php
		}
	}
	?>
...
if (VikRentItems::allowRent()) {
	...
	$coordsplaces = array();
	$selform = "<div class=\"vridivsearch vri-main-search-form\">".$deliveryservicetext."<form action=\"".JRoute::_('index.php?option=com_vikrentitems'.(!empty($pitemid) ? '&Itemid='.$pitemid : ''))."\" method=\"get\" onsubmit=\"return vriValidateSearch();\"><div class=\"vricalform\">\n";
...
jQuery(function() {
	...
	if (count($coordsplaces) > 0) {
		$selform = '<div class="vrilocationsbox"><div class="vrilocationsmapdiv"><a href="'.JRoute::_('index.php?option=com_vikrentitems&view=locationsmap&elemid='.$item['id'].'&tmpl=component').'" class="vrimodalframe" target="_blank"><i class="' . VikRentItemsIcons::i('map-marked') . '"></i><span>'.JText::_('VRILOCATIONSMAP').'</span></a></div></div>'.$selform;
	}
	//
	echo $selform;
	?>
