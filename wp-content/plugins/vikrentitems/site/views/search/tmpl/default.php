...
foreach ($res as $k => $r) {
	...
	?>
		<div class="vri-search-result-block<?php echo count($res_extra_classes) ? (' ' . implode(' ', $res_extra_classes)) : ''; ?>">
			<?php
			/**
			 * @wponly 	if the Itemid is missing, maybe because of a redirect, then using JRoute::_('index.php?option=com_vikrentitems')
			 * 			generated an empty URL (the home page), by losing the navigation and by rendering an invalid page.
			 * 			So we need to use JRoute::_('index.php?option=com_vikrentitems&view=vikrentitems') like the link above.
			 * 			Also, the method of the form has to be POST.
			 */
			?>
			<form action="<?php echo JRoute::_('index.php?option=com_vikrentitems&view=vikrentitems'.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" method="post">
				...
				<div class="vri-result-item-cont">
					...
					?>
						<div class="vri-search-subdiv">
							<button type="submit" class="btn vricontinue"><?php echo JText::_('VRPROSEGUI'); ?></button>
						</div>
					</div>
				</div>
			<?php
