		...
		if ($getjson || VikRentItems::allowRent()) {
			...
			if (!empty($ppickupdate) && !empty($preleasedate)) {
				if (VikRentItems::dateIsValid($ppickupdate) && VikRentItems::dateIsValid($preleasedate)) {
								...
								if (empty($errclosingdays)) {
									...
									if (count($arrtar)) {
										...
										if (!$getjson && !empty($pitemdetail) && array_key_exists($pitemdetail, $arrtar)) {
											...
											$mainframe=JFactory::getApplication();
											$mainframe->redirect(JRoute::_("index.php?option=com_vikrentitems&task=showprc&itemopt=" . $pitemdetail . "&days=" . $daysdiff . "&pickup=" . $first . "&release=" . $second . "&place=" . $pplace . "&returnplace=" . $returnplace . "&itemquant=" . $pitemquant . (is_array($usetimeslot) && count($usetimeslot) > 0 ? "&timeslot=".$usetimeslot['id'] : "") . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
										} else {
										...
										$this->setVriError((isset($restrictionerrmsg) && !empty($restrictionerrmsg) ? $restrictionerrmsg : JText::_('VRNOITEMSINDATE')));
									}
								} else {
									//closing days error
					...
					$this->setVriError(JText::_('VRWRONGDF') . ": " . VikRentItems::sayDateFormat());
				}
			} else {
				$this->setVriError(JText::_('VRSELPRDATE'));
			}
		} else {
