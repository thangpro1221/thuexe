...
<div class="vriprintdiv">
	<a href="<?php echo JRoute::_('index.php?option=com_vikrentitems&view=order&sid='.$ord['sid'].'&ts='.$ord['ts'].'&printer=1&tmpl=component'.(!empty($bestitemid) ? '&Itemid='.$bestitemid : (!empty($pitemid) ? '&Itemid='.$pitemid : ''))); ?>" target="_blank" title="<?php echo addslashes(JText::_('VRIPRINTCONFORDER')); ?>">
...
<div class="vri-ord-top-container">
	...
	if ($ord['status'] == 'confirmed') {
		?>
		<h4><?php VikRentItemsIcons::e('check-circle'); ?> <?php echo JText::_('VRIYOURORDISCONF'); ?></h4>
		<?php
	} elseif ($ord['status'] == 'cancelled') {
		?>
		<h4><?php VikRentItemsIcons::e('times-circle'); ?> <?php echo JText::_('VRIYOURORDISCANC'); ?></h4>
		<?php
	} else {
		?>
		<h4><?php VikRentItemsIcons::e('exclamation-triangle'); ?> <?php echo JText::_('VRIYOURORDISPEND'); ?></h4>
		<?php
	}
	?>
	</div>
