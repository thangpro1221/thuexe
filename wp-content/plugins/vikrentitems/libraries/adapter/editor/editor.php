...
abstract class JEditor
{
	 ...
	 * @param   string   $id       An optional ID for the textarea (note: since 10.1.20). If not supplied the name is used.
	 *
	 * @return  string 	 The editor.
	 *
	 * @link 	https://codex.wordpress.org/Function_Reference/wp_editor
	 */
	public function display($name, $html, $width, $height, $col, $row, $buttons = true, $id = null)
	{
		$width 	= max(array(intval($width), 400));
		$height = max(array(intval($height), 300));
		$id     = preg_replace("/[^a-zA-Z0-9_]+/", '_', $id ? $id : $name);

		ob_start();

		$this->render($name, $html, $width, $height, $col, $row, $buttons, $id);
	 ...
	 * @return  string 	 The editor.
	 */
	abstract protected function render($name, $html, $width, $height, $col, $row, $buttons, $id);
}
