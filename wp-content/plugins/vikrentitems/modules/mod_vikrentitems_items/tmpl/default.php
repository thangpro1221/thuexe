...
JHtml::_('script', $baseurl . 'modules/mod_vikrentitems_items/src/owl.carousel.min.js');
				...
				<div class="vrimoditemsboxdiv">
					...
					<div class="vriinf">
						...
						if ($c['cost'] > 0) {
							...
							if (!empty($cost_text)) {
								?>
								<span class="vrimoditemsitemcost-txt"><?php echo JText::_($cost_text); ?></span> 
								<?php
							}
							?>
							</div>
					...
					<span class="vrimoditemsview">
						<a href="<?php echo JRoute::_('index.php?option=com_vikrentitems&view=itemdetails&elemid='.$c['id'].'&Itemid='.$params->get('itemid')); ?>"><?php echo JText::_('VRIMODITEMSCONTINUE'); ?></a>
					</span>
						<?php
					}
					?>
...
jQuery(document).ready(function() { 
	jQuery("#vri-moditems-<?php echo $randid; ?>").owlCarousel({
		...
		navText : ['<?php echo JText::_('VRIMODITEMSPREV'); ?>', '<?php echo JText::_('VRIMODITEMSNEXT'); ?>'],
