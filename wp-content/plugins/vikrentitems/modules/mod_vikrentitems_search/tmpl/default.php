...
function vrimodGlobalClosingDays(date) {
	...
	if (jQuery.inArray(gdmy, checksdarr) == -1 && jQuery.inArray(gwd, checkwdarr) == -1) {
		...
		return [false, "", "'.addslashes(JText::_('VRIMGLOBDAYCLOSED')).'"];
	}
}';
...
function vriSetLocOpenTimeModule(loc, where) {
	...
	jQuery.ajax({
		type: "POST",
		url: "'.JRoute::_('index.php?option=com_vikrentitems&task=ajaxlocopentime&tmpl=component&Itemid=' . $params->get('itemid', 0)).'",
		...
		if (where == "pickup" && vrimod_location_change === false) {
			...
			vrimod_location_change = false;
				...
				$vrloc .= "<div class=\"vrisfentrymod\"><label for=\"modplace\">".JText::_('VRMPPLACE')."</label><span class=\"vriplacesp\"><select name=\"place\" id=\"modplace\"".$onchangeplaces.">";
...
function modpickupClosingDays'.$randid.'(date) {
	...
	if (jQuery.inArray(dmy, checklocarr) == -1) {
		...
		return [false, "", "'.addslashes(JText::_('VRIMLOCDAYCLOSED')).'"];
	}
}
...
<div class="<?php echo $params->get('moduleclass_sfx'); ?>">
	...
	?>
		<form action="<?php echo JRoute::_('index.php?option=com_vikrentitems&Itemid='.$params->get('itemid', 0)); ?>" method="post" onsubmit="return vriValidateSearch<?php echo $randid; ?>();">
			...
			<div class="vrimodcalform">
				...
				if (is_array($vrisessioncart) && count($vrisessioncart) > 0) {
					echo "<div class=\"vrisfentrymodsum vri-modsearch-sessvals\"><div class=\"vrisearchmoddivpickup\"><span class=\"vrisearchmodspanone\">" . JText::_('VRMPICKUPCAR') . "</span><span class=\"vrisearchmodspantwo\"><input type=\"hidden\" name=\"pickupdate\" id=\"pickupdatemod".$randid."\" value=\"".date($jsdf, $vrisesspickup)."\"/>".date($jsdf, $vrisesspickup)." " . (!empty($nowtf) ? JText::_('VRMALLE') : '') . " <input type=\"hidden\" name=\"pickuph\" value=\"".date('H', $vrisesspickup)."\"/>".(!empty($nowtf) ? date('H', $vrisesspickup).":" : '')."<input type=\"hidden\" name=\"pickupm\" value=\"".date('i', $vrisesspickup)."\"/>".(!empty($nowtf) ? date('i', $vrisesspickup) : '')."</span></div></div>\n";
					echo "<div class=\"vrisfentrymodsum vri-modsearch-sessvals\"><div class=\"vrisearchmoddivdropoff\"><span class=\"vrisearchmodspanone\">" . JText::_('VRMRETURNCAR') . "</span><span class=\"vrisearchmodspantwo\"><input type=\"hidden\" name=\"releasedate\" id=\"releasedatemod".$randid."\" value=\"".date($jsdf, $vrisessdropoff)."\"/>".date($jsdf, $vrisessdropoff)." " . (!empty($nowtf) ? JText::_('VRMALLE') : '') . " <input type=\"hidden\" name=\"releaseh\" value=\"".date('H', $vrisessdropoff)."\"/>".(!empty($nowtf) ? date('H', $vrisessdropoff) : '')."<input type=\"hidden\" name=\"releasem\" value=\"".date('i', $vrisessdropoff)."\"/>".(!empty($nowtf) ? date('i', $vrisessdropoff) : '')."</span></div></div>";
				}
				//
				...
				if (true) {
					if (!is_array($vrisessioncart) || !count($vrisessioncart)) {
						echo "<div class=\"vrisfentrymod vrisfentrymod-dpicker\"><label class=\"vripickdroplab\" for=\"pickupdatemod".$randid."\">" . JText::_('VRMPICKUPCAR') . "</label><div class=\"vrisfentrydate\"><input type=\"text\" name=\"pickupdate\" id=\"pickupdatemod".$randid."\" size=\"10\" autocomplete=\"off\"/><i class=\"fas fa-calendar-alt vri-caltrigger\"></i></div>";
						if (is_array($timeslots) && count($timeslots) > 0) {
							echo "<div class=\"vrisfentrymodtimeslot\"><label for=\"timeslotsmod\">".JText::_('VRIMFOR') . "</label>";
							...
							echo "<div class=\"vrisfentrymodtime\"><label for=\"modpickuph\">".JText::_('VRMALLE') . "</label><span id=\"vrimodselph\">" . $selpickh . "</span><label class=\"vritimedots\">:</label><span id=\"vrimodselpm\">".$selpickm."</span></div></div>\n";
							...
							echo "<div class=\"vrisfentrymod vrisfentrymod-dpicker\"><label class=\"vripickdroplab\" for=\"releasedatemod".$randid."\">" . JText::_('VRMRETURNCAR') . "</label><div class=\"vrisfentrydate\"><input type=\"text\" name=\"releasedate\" id=\"releasedatemod".$randid."\" size=\"10\" autocomplete=\"off\"/><i class=\"fas fa-calendar-alt vri-caltrigger\"></i></div><div class=\"vrisfentrymodtime\"><label for=\"modreleaseh\">" . JText::_('VRMALLE') . "</label><span id=\"vrimodseldh\">".$seldroph."</span><label class=\"vritimedots\">:</label><span id=\"vrimodseldm\">".$seldropm."</span></div></div>";
						}
					}
				} else {
					/**
				...
				if (@is_array($places)) {
					$vrlocreturn="";
					$vrlocreturn .= "<div class=\"vrisfentrymod\"><label for=\"modreturnplace\">".JText::_('VRMPLACERET')."</label><span class=\"vriplacesp\"><select name=\"returnplace\" id=\"modreturnplace\"".(strlen($onchangeplacesdrop) > 0 ? $onchangeplacesdrop : "").">";
					foreach($places as $pla) {
				...
				if (intval($params->get('showcat')) == 1) {
					...
					if ($dbo->getNumRows() > 0) {
						...
						$vricats .= "<div class=\"vrisfentrymod\"><label for=\"modcategories\">".JText::_('VRMCARCAT')."</label><span class=\"vriplacesp\"><select name=\"categories\" id=\"modcategories\">";
						$vricats .= "<option value=\"all\">".JText::_('VRMALLCAT')."</option>\n";
				...
				<div class="vrisfentrymodsubmit">
					<button type="submit" class="btn vrisearch"><?php echo JText::_('SEARCHD'); ?></button>
				</div>
			</div>
		</form>
		...
		<div class="vrisearchmodgosummarydiv">
			<a class="btn" href="<?php echo JRoute::_('index.php?option=com_vikrentitems&task=oconfirm&place='.$vrisesspickuploc.'&returnplace='.$vrisessdropoffloc.'&days='.$vrisessdays.'&pickup='.$vrisesspickup.'&release='.$vrisessdropoff.'&Itemid='.$params->get('itemid', 0)); ?>"><?php echo JText::_('VRIMGOTOSUMMARY'); ?></a>
		</div>
			<?php
		}
		?>
