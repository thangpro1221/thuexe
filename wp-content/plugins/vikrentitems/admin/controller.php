...
class VikRentItemsController extends JControllerVikRentItems {
	...
	function createoptional() {
		...
		if (!empty($poptname)) {
			...
			$app->enqueueMessage(JText::_('VRISUCCUPDOPTION'));
	...
	private function do_createseason($andnew = false) {
		...
		if ((!empty($pfrom) && !empty($pto)) || count($pwdays) > 0) {
			...
			if ($valid || $ppromo === 1) {
				...
				VikError::raiseWarning('', JText::_('ERRINVDATEITEMSLOCSEASON').(count($double_records) > 0 ? ' ('.implode(', ', $double_records).')' : ''));
				$mainframe->redirect("index.php?option=com_vikrentitems&task=newseason");
			}
		} else {
			$mainframe->redirect("index.php?option=com_vikrentitems&task=newseason");
	...
	function setordconfirmed() {
		...
		if ($dbo->getNumRows() == 1) {
			...
			if (!$allbook) {
				...
				// VikRentItems::sendCustMailFromBack($order[0]['custmail'], strip_tags($ftitle)." ".JText::_('VRRENTALORD'), $ftitle, $nowts, $order[0]['custdata'], $vricart, $order[0]['ritiro'], $order[0]['consegna'], $isdue, $viklink, JText::_('VRIOMPLETED'), $ritplace, $consegnaplace, $maillocfee, $order[0]['id'], $order[0]['coupon'], true, $totdelivery);
			}
		}
	...
	function moditemrateplans() {
		...
		if (empty($pid_item) || empty($pid_price) || empty($ptype) || empty($pfromdate) || empty($ptodate) || !(strtotime($pfromdate) > 0)  || !(strtotime($ptodate) > 0)) {
			echo 'e4j.error.'.addslashes(JText::_('VRIRATESOVWERRMODRPLANS'));
			exit;
		}
		$price_record = array();
	...
	function calc_rates() {
		...
		if (defined('ABSPATH')) {
			...
			$endpoint = JRoute::_($endpoint, false);
		}
					...
					if (array_key_exists($id_item, $arr_res)) {
						...
						foreach ($arr_res[$id_item] as $rate) {
							...
							if (array_key_exists('affdays', $rate) && $rate['affdays'] > 0) {
								$extra_response .= '<span class="vri-calcrates-extrapricedet vri-calcrates-ratespaffdays"><span>'.JText::_('VRICALCRATESSPAFFDAYS').'</span>'.$rate['affdays'].'</span>';
							}
							$tot = round($rate['cost'], 2);
							$response .= '<span class="vri-calcrates-pricedet vri-calcrates-ratetotal"><span>'.JText::_('VRICALCRATESTOT').'</span>'.$currencysymb.' '.VikRentItems::numberFormat($tot).'</span>';
	...
	function getordersinfo() {
		...
		if ($dbo->getNumRows() > 0) {
			...
			foreach ($booking_infos as $k => $row) {
				...
				$custdata = JText::_('VRDBTEXTROOMCLOSED') == $row['custdata'] ? '<span class="vriordersitemclosed">'.JText::_('VRDBTEXTROOMCLOSED').'</span>' : $custdata;
	...
	private function do_updatecron($stay = false) {
		...
		$dbo->setQuery($q);
		$dbo->execute();
		$mainframe->enqueueMessage(JText::_('VRICRONUPDATED'));
		if ($stay) {
	...
	private function do_createcron($stay = false) {
		...
		if (!empty($lid)) {
			...
			//
			$mainframe->enqueueMessage(JText::_('VRICRONSAVED'));
			if ($stay) {
