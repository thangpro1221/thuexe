				...
				<div class="vri-params-wrap">
					...
					<div class="vri-params-container">
						<div class="vri-param-container">
							<div class="vri-param-label"><?php echo JText::_('VRNEWCUSTOMFONE'); ?></div>
							<div class="vri-param-setting"><input type="text" name="name" value="<?php echo count($row) ? htmlspecialchars($row['name']) : ''; ?>" size="40"/></div>
						</div>
						<div class="vri-param-container">
							<div class="vri-param-label"><?php echo JText::_('VRNEWCUSTOMFTWO'); ?></div>
						...
						<div class="vri-param-container">
							<div class="vri-param-label"><?php echo JText::_('VRNEWCUSTOMFSIX'); ?></div>
							<div class="vri-param-setting">
								<?php echo $vri_app->printYesNoButtons('required', JText::_('VRYES'), JText::_('VRNO'), (count($row) && intval($row['required']) == 1 ? 1 : 0), 1, 0); ?>
							</div>
						</div>
						<div class="vri-param-container" id="vrflag"<?php echo (count($row) && $row['type'] != "text" ? " style=\"display: none;\"" : ""); ?>>
							<div class="vri-param-label"><?php echo JText::_('VRNEWCUSTOMFFLAG'); ?> <?php echo $vri_app->createPopover(array('title' => JText::_('VRNEWCUSTOMFFLAG'), 'content' => JText::_('VRNEWCUSTOMFFLAGHELP'))); ?></div>
							<div class="vri-param-setting">
								<select name="flag">
									<option value=""></option>
									<option value="isemail"<?php echo (count($row) && intval($row['isemail']) == 1 ? " selected=\"selected\"" : ""); ?>><?php echo JText::_('VRNEWCUSTOMFSEVEN'); ?></option>
									<option value="isnominative"<?php echo (count($row) && intval($row['isnominative']) == 1 ? " selected=\"selected\"" : ""); ?>><?php echo JText::_('VRIISNOMINATIVE'); ?></option>
									<option value="isphone"<?php echo (count($row) && intval($row['isphone']) == 1 ? " selected=\"selected\"" : ""); ?>><?php echo JText::_('VRIISPHONENUMBER'); ?></option>
									<option value="isaddress"<?php echo (count($row) && stripos($row['flag'], 'address') !== false ? " selected=\"selected\"" : ""); ?>><?php echo JText::_('VRIISADDRESS'); ?></option>
									<option value="iscity"<?php echo (count($row) && stripos($row['flag'], 'city') !== false ? " selected=\"selected\"" : ""); ?>><?php echo JText::_('VRIISCITY'); ?></option>
									<option value="iszip"<?php echo (count($row) && stripos($row['flag'], 'zip') !== false ? " selected=\"selected\"" : ""); ?>><?php echo JText::_('VRIISZIP'); ?></option>
									<option value="iscompany"<?php echo (count($row) && stripos($row['flag'], 'company') !== false ? " selected=\"selected\"" : ""); ?>><?php echo JText::_('VRIISCOMPANY'); ?></option>
									<option value="isvat"<?php echo (count($row) && stripos($row['flag'], 'vat') !== false ? " selected=\"selected\"" : ""); ?>><?php echo JText::_('VRIISVAT'); ?></option>
								</select>
							</div>
						</div>
						<div class="vri-param-container">
							<div class="vri-param-label"><?php echo JText::_('VRNEWCUSTOMFEIGHT'); ?></div>
							<div class="vri-param-setting">
