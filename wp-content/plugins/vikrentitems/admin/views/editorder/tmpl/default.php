	...
	<form name="adminForm" id="adminForm" action="index.php" method="post">	
		...
		<div class="vri-bookingdet-innertop">
			...
			if ((is_array($items) && !empty($items[0]['idtar'])) || $is_cust_cost) {
				...
				$order_uri 	= '';
				if ($itemid) {
					$order_uri = JRoute::_("index.php?option=com_vikrentitems&Itemid={$itemid}&view=order&sid={$row['sid']}&ts={$row['ts']}");
				} else {
				...
				?>
				<div class="vri-bookingdet-command">
					<button onclick="window.open('<?php echo $order_uri; ?>', '_blank');" type="button" class="btn btn-secondary"><i class="icon-eye"></i> <?php echo JText::_('VRIVIEWORDFRONT'); ?></button>
				</div>
				<?php
			}
			...
			<div class="vri-bookingdet-innercontainer">
				...
				?>
					<div class="vri-bookingdet-detcont vri-hidein-print">
						...
						<?php if (!empty($row['custmail'])) : ?> <button type="button" class="btn vri-config-btn" onclick="vriToggleSendEmail();" style="vertical-align: top;"><i class="vriicn-envelop"></i><?php echo JText::_('VRSENDEMAILACTION'); ?></button><?php endif; ?>
					</div>
				<?php
				}
				?>
				...
				<div class="vri-bookingdet-noteslogs">
					...
					<div class="vri-bookingdet-noteslogs-cont">
						<div id="vriadminnotesdiv" style="display: block;">
							...
							<br clear="all"/>
							<input class="btn btn-success vri-config-btn" type="submit" name="updadmnotes" value="<?php echo JText::_('VRIADMINNOTESUPD'); ?>" />
						</div>
					<?php
					if (!empty($row['paymentlog'])) {
						?>
