	...
	<div class="vikwppro-header">
		<div class="vikwppro-header-inner">
			<div class="vikwppro-download-result">
				<h2><?php VikRentItemsIcons::e('refresh', 'fa-spin'); ?> <?php echo JText::_('VRIPROPLWAIT'); ?></h2>
				<h3 class="vikwppro-cur-action"><?php echo JText::_('VRIPRODLINGPKG'); ?></h3>
			</div>
...
function vikwpStopMonitoring(result) {
	...
	if (result === true) {
		var continuebtn = '<p><button type="button" class="button button-primary" onclick="document.location.href=\'admin.php?option=com_vikrentitems\';"><?php echo addslashes(JText::_('VRIUPDCOMPLOKCLICK')); ?></button></p>';
		jQuery('.vikwppro-download-result').html('<h1 class="vikwp-download-success"><?php VikRentItemsIcons::e('check'); ?></h1>'+continuebtn);
	} else {
		var continuebtn = '<p><button type="button" class="button" onclick="document.location.href=\'admin.php?option=com_vikrentitems&view=gotopro\';"><?php echo addslashes(JText::_('VRIUPDCOMPLNOKCLICK')); ?></button></p>';
