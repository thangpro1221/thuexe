...
<form action="index.php?option=com_vikrentitems&task=orders" method="post" name="adminForm" id="adminForm" class="vri-allorders-fm">
		...
		<div id="vri-search-tools-cont" class="js-stools-container-filters clearfix" style="display: none;">
			...
			<div class="btn-group pull-left vri-sel2-font">
				...
				?>
					...
					<option value="closure"<?php echo $pstatus == 'closure' ? ' selected="selected"' : ''; ?>><?php echo JText::_('VRDBTEXTROOMCLOSED'); ?></option>
				</select>
			</div>
			<?php
			if (is_array($all_locations)) {
	...
	<div class="table-responsive">
		...
		for ($i = 0, $n = count($rows); $i < $n; $i++) {
			...
			$custdata = $row['closure'] > 0 || JText::_('VRDBTEXTROOMCLOSED') == $row['custdata'] ? '<span class="vriordersitemclosed"><i class="'.VikRentItemsIcons::i('ban').'"></i> '.JText::_('VRDBTEXTROOMCLOSED').'</span>' : $custdata;
			//
			$status_lbl = '';
