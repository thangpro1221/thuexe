				...
				<td style="text-align: center;">
					<?php if ($row->post_id) { ?>
						...
						<a href="<?php echo get_permalink($row->post_id); ?>" target="_blank" class="btn btn-primary vri-link-btn-small">
							<?php echo JText::_('VRI_SC_VIEWFRONT'); ?> <i class="<?php echo VikRentItemsIcons::i('external-link-square'); ?>"></i>
						</a>
					...
					<?php } else if ($row->tmp_post_id) { 
						...
						<a href="edit.php?post_status=trash&post_type=<?php echo $post->post_type; ?>" target="_blank" class="btn vri-link-btn-small">
							<?php echo JText::_('VRI_SC_VIEWTRASHPOSTS'); ?> <i class="<?php echo VikRentItemsIcons::i('external-link-square'); ?>" style="color: #900;"></i>
						</a>

					<?php } else { ?>

						<a href="index.php?option=com_vikrentitems&task=shortcode.add_to_page&sc_id=<?php echo $row->id; ?>&return=<?php echo $this->returnLink; ?>" class="btn btn-danger vri-link-btn-small" onclick="return confirm('<?php echo $this->escape(JText::_('VRI_SC_ADDTOPAGE_HELP')); ?>');">
							<?php echo JText::_('VRI_SC_ADDTOPAGE'); ?> <i class="<?php echo VikRentItemsIcons::i('plus-square'); ?>"></i>
						</a>
